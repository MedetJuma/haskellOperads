...
The `-N` flag can be adjusted by appending a number of processors, which by default is set to infinity. For example, `-N30` means that the process will use at most 30 processors. The `-A64m` flag specifies the garbage collector size, which is set to 64 MB.
